#!/usr/bin/env python3
"""Smoke5 stability concurrent driver — 276 × {B01,B02,N6,B33,B36} × rep1-3.

Task t_a7b64411 (correct contract): ONE book (276) × 5 metrics × 3 reps = 15
real calls against commandcode/deepseek-v4-flash config-4
(reasoning off, max_tokens 32768, temperature 0, top_p 1, seed 0).
Same book scored 3 independent times to measure scoring consistency/stability.

Reuses the frozen isolation machinery from stage_only_exec.py: per-card
staging root, sanitized child env, --no-http child scoring, atomic
write_staged_raw, staging-only fan-in. Does NOT modify any spec/prompt/
config/frozen artifact. New isolated run dir:
  runs/dev10_v5_r3_iso_v4_smoke5_stability_276_3rep
The superseded 3-book run (dev10_v5_r3_iso_v4_smoke5_stability_3x5*) stays
untouched (audit only per Director).
"""
...
_STAGE_SPEC = importlib.util.spec_from_file_location("smoke5_stability_276_3rep_stage", _STAGE_MOD)
...
# Correct contract: 1 book × 5 metrics × 3 reps = 15 real calls.
BOOKS = ("276",)
...
def call_commandcode(prompt: str, cfg: dict, api_key: str, base_url: str) -> tuple[str, dict]:
    ...
    Same transport as run_dev10.live_acquire_parent_raw / smoke5 r5:
    OpenAI-compatible endpoint, browser-class UA for Cloudflare, json_object
    response format, temperature 0 / top_p 1 / seed 0 frozen from staged cfg.
    When reasoning_mode is off the payload does NOT include reasoning_effort
    or extra_body.thinking (DeepSeek V4 wire). Never logs credentials.
    """
    import urllib.error
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"workers={workers} books={list(BOOKS)} metrics={list(METRICS)} reps={list(REPS)}",
        flush=True,
    )
